...
void enc28j60_init(BYTE *avr_mac) {
    ...
    // Late collisions occur beyond 63+8 bytes (8 bytes for preamble/start of
    // frame delimiter)
    // 55 is all that is needed for IEEE 802.3, but ENC28J60 B5 errata for
    // improper link pulse
...
//
//*****************************************************************************
BYTE enc28j60getrev(void) { return (enc28j60Read(EREVID)); }
